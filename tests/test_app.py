...
class TestGenerateAppYaml:
    """Tests for generate_app_yaml."""

    def setup_method(self) -> None:
        reset_registries()

    def test_includes_command(self) -> None:
        yaml_text = generate_app_yaml()

        assert "command:" in yaml_text
        assert "  - python" in yaml_text
        assert "  - app.py" in yaml_text

    def test_includes_env_vars_with_value_from(self) -> None:
        _JOB_REGISTRY["etl_daily"] = JobMeta(
            fn=_dummy_fn,
            name="etl_daily",
            dag={},
        ...
        assert "  - name: DBXDEC_JOB_ETL_DAILY" in yaml_text
        assert "    valueFrom: etl-daily" in yaml_text

    def test_empty_registry_no_env(self) -> None:
        yaml_text = generate_app_yaml()

        assert "env:" not in yaml_text

    def test_multiple_jobs_sorted(self) -> None:
        _JOB_REGISTRY["z_job"] = JobMeta(fn=_dummy_fn, name="z_job", dag={})
        _JOB_REGISTRY["a_job"] = JobMeta(fn=_dummy_fn, name="a_job", dag={})

        yaml_text = generate_app_yaml()

        a_pos = yaml_text.index("DBXDEC_JOB_A_JOB")
        z_pos = yaml_text.index("DBXDEC_JOB_Z_JOB")
        assert a_pos < z_pos

"""Tests for PolarsJsonIoManager using real Polars I/O on local files."""
...
class TestConstruction:
    ...
    def test_base_path_as_callable(self) -> None:
        io = PolarsJsonIoManager(base_path=lambda: "/data/lake/")
        assert io.base_path == "/data/lake"
    ...
    def test_base_path_callable_invoked_each_time(self) -> None:
        ...
        io = PolarsJsonIoManager(base_path=_factory)
...
class TestRoundTrip:
    def test_lazyframe_round_trip(self, tmp_path: Path) -> None:
        io = PolarsJsonIoManager(base_path=str(tmp_path))
    ...
    def test_dataframe_round_trip(self, tmp_path: Path) -> None:
        """DataFrame is converted to lazy internally then sink_ndjson is called."""
        io = PolarsJsonIoManager(base_path=str(tmp_path))
    ...
    def test_write_creates_ndjson_file(self, tmp_path: Path) -> None:
        io = PolarsJsonIoManager(base_path=str(tmp_path))
        io.write(_output_ctx("extract"), _SAMPLE.lazy())
        assert (tmp_path / "extract.ndjson").exists()
...
class TestReadDispatch:
    def test_defaults_to_lazyframe(self, tmp_path: Path) -> None:
        io = PolarsJsonIoManager(base_path=str(tmp_path))
        _SAMPLE.lazy().sink_ndjson(tmp_path / "t.ndjson")
        result = io.read(_input_ctx("t"))
        assert isinstance(result, pl.LazyFrame)
...
class TestOptions:
    ...
    def test_read_options_n_rows(self, tmp_path: Path) -> None:
...
class TestPartitioning:
    ...
    def test_dataframe_partitioned_routes_through_lazy(self, tmp_path: Path) -> None:
        """DataFrame partition goes through .lazy().sink_ndjson(PartitionBy)."""
        io = PolarsJsonIoManager(base_path=str(tmp_path))

"""Tests for PolarsCsvIoManager using real Polars I/O on local files."""
...
class TestConstruction:
    ...
    def test_base_path_as_callable(self) -> None:
        io = PolarsCsvIoManager(base_path=lambda: "/data/lake/")
        assert io.base_path == "/data/lake"
    ...
    def test_base_path_callable_invoked_each_time(self) -> None:
        ...
        io = PolarsCsvIoManager(base_path=_factory)
...
class TestRoundTrip:
    def test_dataframe_round_trip(self, tmp_path: Path) -> None:
        io = PolarsCsvIoManager(base_path=str(tmp_path))
    ...
    def test_write_creates_csv_file(self, tmp_path: Path) -> None:
        ...
        assert (tmp_path / "extract.csv").exists()
...
class TestReadDispatch:
    def test_defaults_to_lazyframe(self, tmp_path: Path) -> None:
        io = PolarsCsvIoManager(base_path=str(tmp_path))
        _SAMPLE.write_csv(tmp_path / "t.csv")
        result = io.read(_input_ctx("t"))
        assert isinstance(result, pl.LazyFrame)
...
class TestOptions:
    def test_write_options_applied(self, tmp_path: Path) -> None:
        ...
        raw = (tmp_path / "t.csv").read_text()
        assert ";" in raw

    def test_read_options_applied(self, tmp_path: Path) -> None:
        # Write with semicolon, read with matching option.
        _SAMPLE.write_csv(tmp_path / "t.csv", separator=";")
        io = PolarsCsvIoManager(
            base_path=str(tmp_path),
...
class TestPartitioning:
    ...
    def test_dataframe_partitioned_creates_hive_dirs(self, tmp_path: Path) -> None:
        """DataFrame partition goes through .lazy().sink_csv(PartitionBy)."""
        io = PolarsCsvIoManager(base_path=str(tmp_path))

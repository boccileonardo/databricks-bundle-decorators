    ...
    *,
    ...
    return {
        resource_key: {
            ...
            # Do NOT set config.env here — env vars with valueFrom
            # must be in app.yaml (app source dir), not in the bundle
            # resource config.  The bundle config.env only supports
            # literal name/value pairs and would override app.yaml.
            "config": {
...
def generate_app_yaml() -> str:
    """Return the ``app.yaml`` content with ``valueFrom`` env vars.

    The ``app.yaml`` lives inside the ``app/`` source directory and
    is read by the Databricks Apps runtime.  Each registered job gets
    a ``DBXDEC_JOB_<NAME>`` env var whose ``valueFrom`` references
    the corresponding app resource binding name.
    """
    lines = [
        "# Auto-generated by: dbxdec init --dashboard",
        "# Regenerate with:   dbxdec app-config",
    ...
    if job_names:
        ...
        for job_name in job_names:
            ...
            resource_name = job_name.replace("_", "-")
            lines.append(f"  - name: {env_var_name}")
            lines.append(f"    valueFrom: {resource_name}")
